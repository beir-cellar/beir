...
class CohereEmbedAPI:
    def __init__(
        ...
        **kwargs,
        ...
        self.co = cohere.ClientV2(os.getenv("COHERE_API_KEY") if api_key is None else api_key)
        ...
        for start_idx in trange(0, len(texts), batch_size):
            ...
            try:
                response = self.co.embed(
                    texts=sub_texts, model=self.model_path, input_type=input_type, embedding_types=["float"]
                )
