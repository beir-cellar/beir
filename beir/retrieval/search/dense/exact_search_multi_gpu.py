    ...
    class DummyMetric(EvaluationModule):
        ...
        def _info(self):
            return EvaluationModuleInfo(
                ...
                features=Features(
                    {
                        "cos_scores_top_k_values": Array2D((None, self.len_queries), "float32"),
                        "cos_scores_top_k_idx": Array2D((None, self.len_queries), "int32"),
                        "batch_index": Value("int32"),
                    },
                ),
            )
        ...
        def warmup(self):
            ...
            """
            self.add_batch(
                cos_scores_top_k_values=torch.ones((1, 1, self.len_queries), dtype=torch.float32),
                cos_scores_top_k_idx=torch.ones((1, 1, self.len_queries), dtype=torch.int32),
                batch_index=-torch.ones(1, dtype=torch.int32),
            )
    ...
    def search(
        ...
        **kwargs,
        ...
        if importlib.util.find_spec("evaluate") is None:
            raise ImportError(
                "evaluate library not available. Please do ``pip install evaluate`` library with Python>=3.7 (not available with Python 3.6) to use distributed and multigpu evaluation."
            )

        self.corpus_chunk_size = (
            min(math.ceil(len(corpus) / len(self.target_devices) / 10), 5000)
            if self.corpus_chunk_size is None
            else self.corpus_chunk_size
        )
        self.corpus_chunk_size = min(
            self.corpus_chunk_size, len(corpus) - 1
        )  # to avoid getting error in metric.compute()

        if self.sort_corpus:
            logger.info("Sorting Corpus by document length (Longest first)...")
            corpus = corpus.map(
                lambda x: {"len": len(x.get("title", "") + x.get("text", ""))},
                num_proc=4,
            )
        ...
        metric = DummyMetric(
            ...
            num_process=len(self.target_devices),
            process_id=0,
        )
        metric.filelock = FileLock(
            os.path.join(
                metric.data_dir,
                f"{metric.experiment_id}-{metric.num_process}-{metric.process_id}.arrow.lock",
            )
        )
        metric.cache_file_name = os.path.join(
            metric.data_dir,
            f"{metric.experiment_id}-{metric.num_process}-{metric.process_id}.arrow",
        )
        ...
        with torch.no_grad():
            while True:
                try:
                    ...
                    cos_scores = self.score_functions[self.score_function](
                        self.query_embeddings.to(corpus_embeds.device), corpus_embeds
                    ).detach()
                    ...
                    cos_scores_top_k_values, cos_scores_top_k_idx = torch.topk(
                        cos_scores,
                        min(self.top_k + 1, len(cos_scores[1])),
                        dim=1,
                        largest=True,
                        sorted=False,
                    )
                    ...
                    metric.add_batch(
                        ...
                        batch_index=[id] * len(cos_scores_top_k_values),
                    )
