...
from re_utils import MSMARCO_dataset, msmarco_collate_fn, llm_output, reward_computing, minibatch_indices, state_to_action_probs, CriticModel
...
if __name__=='__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (queries, gt_paragraphs, scores, gt_answer) in pbar:
            
            with torch.no_grad():
                # (B x M), (B x M), list of M passages, (B x D), (M x D)
                log_prob_sim_matrix, sim_matrix, unique_passages, E_q, E_p_unique = state_to_action_probs(queries, gt_paragraphs, retrieval_model)
            ...
            rewards = reward_computing(llm_answers, gt_answer).to(accelerator.device)     # (B, )
            ...
            for idx in minibatch_indices(real_batch_size, minibatch_size):
                ...
                # 1. Sample a minibatch of queries and deduplicated facts. 
                mini_queries = [queries[index] for index in idx]        # list of b questions
                ...
                # 2. Retrive the passages with current policy. Compute probabilities of old actions based on current policy.
                log_prob_sim_matrix, sim_matrix, unique_passages, E_q, E_p_unique = state_to_action_probs(mini_queries, mini_gt_paragraphs, retrieval_model)
